...
def crab_fuel(chosen_crab, crabs, fuel_factor=lambda x: x):
    fuel = 0
    for crab in crabs:
        fuel += fuel_factor(abs(crab - chosen_crab)) * crabs[crab]
    return fuel
...
def part_1(crabs):
    fuel_used = Counter(
        {crab: crab_fuel(crab, crabs) for crab in range(max(crabs) + 1)})
    return fuel_used.most_common()[-1]


def part_2(crabs):
    fuel_used = Counter(
        {crab: crab_fuel(crab, crabs, fuel_factor=triangular_number) for crab in range(max(crabs) + 1)})
    return fuel_used.most_common()[-1]
